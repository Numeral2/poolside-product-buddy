...
const Hero = ({
  ...
  return <div className="relative h-[50vh] md:h-[70vh] w-full overflow-hidden bg-background">
      {/* Lighter overlay */}
      ...
      {/* Video on Right Side */}
      <div className={`absolute right-0 top-0 h-full w-full md:w-1/2 transition-all duration-[2000ms] ease-in-out ${videoEnded ? 'translate-x-full opacity-0 scale-110' : 'translate-x-0 opacity-100 scale-100'}`} style={{
      zIndex: 2
    }}>
        <video ref={videoRef} autoPlay muted playsInline onEnded={handleVideoEnd} className="w-full h-full object-cover">
          <source src="/pool-entry-video.mp4" type="video/mp4" />
        </video>
      </div>

      {/* Product Showcase - Shows after video ends - RIGHT SIDE */}
      {videoEnded && <div className="absolute top-0 h-full flex items-center animate-fade-in transition-all duration-500" style={{
      zIndex: 2,
      ...
      justifyContent: 'flex-start'
          ...
          <div className="flex flex-col items-center gap-2 md:gap-3 lg:gap-4 px-2">
            ...
            <Link to="/products" className="group flex items-center gap-1 sm:gap-1.5 md:gap-2 text-foreground hover:text-primary transition-colors duration-300 text-[10px] xs:text-xs sm:text-sm md:text-base no-underline">
      ...
      <div className="absolute top-0 h-full flex items-center transition-all duration-500" style={{
      zIndex: 10,
      ...
      paddingLeft: catalogOpen ? '0' : 'clamp(1rem, 4vw, 3rem)'
        ...
        <div className="max-w-md space-y-2 sm:space-y-3 md:space-y-4 pt-8 sm:pt-12 md:pt-16 lg:pt-12 px-[11px] py-[50px] mx-[111px]">
          <div className="space-y-1.5 sm:space-y-2 md:space-y-3">
            ...
            <img src={astralpool} alt="AstralPool" className="h-6 xs:h-7 sm:h-8 md:h-10 lg:h-12 object-contain opacity-90" />
          </div>

          {/* Animated Stats - Show only after video ends */}
          {videoEnded && <div className="space-y-1.5 sm:space-y-2 md:space-y-3 pt-2 sm:pt-3 md:pt-4 animate-fade-in">
